...
var adbot = {
    ...
    collect_ad_elements: function () {
        ...
        ad_elements = ad_elements.filter(function(i) {
            ...
            if ( ADBOTjQuery(current_ad).parents("a").length + ADBOTjQuery(current_ad).children("a").length < 1 ) { 
    ...
    ad_link_url: function( ad_element ) {
        ...
        } else if ( ad_element.children("a").length > 0 ) {
            link_url = ad_element.children("a")[0].href;
        } else if ( ad_element.is("object,embed") ) {
            link_url = "object.embed.link.url.TDB";
        } else {
